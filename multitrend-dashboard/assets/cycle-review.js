...
(() => {
  ...
  el('catalogo').querySelector('.lede').textContent = 'Cada fila reúne una publicación y sus variantes. Abrí el detalle para ver todos los atributos vacíos y en qué hoja, fila y SKU revisarlos. Ventas se cruza por la publicación vendida, sin repetir el importe en cada variante.';
  mountTable({table: 'catalog-table', title: 'Campos de catálogo por publicación', search: 'catalog-search', rows: catalogRows, sort: 'income', unit: 'publicaciones', searchKeys: ['title', 'sku', 'item', 'category', 'missing_fields'], filters: [{id: 'catalog-filter', label: 'Ventas', get: row => row.has_net_sales ? 'Con ventas' : 'Sin ventas'}, {id: 'catalog-completion', label: 'Ficha', get: row => row.missing_count > 0 ? 'Con campos pendientes' : 'Completa'}, {id: 'catalog-type', label: 'Tipo', get: row => row.catalog ? 'Catálogo' : 'Propia'}], columns: [
    {key: 'title', label: 'Publicación', format: (value, row) => `<div class="product-cell"><span class="name">${esc(value)}</span><span class="mono">${esc(row.skus.join(' · '))} · ${n(row.variant_rows)} fila(s)</span></div>`}, {key: 'units_net', label: 'Unidades operativas', align: 'r', format: n}, {key: 'income', label: 'Ingresos', align: 'r', format: money}, {key: 'completion_pct', label: 'Completitud', align: 'r', format: pct}, {key: 'missing_count', label: 'Atributos pendientes', align: 'r', format: n}
  ], details: row => detailGrid([['Publicación', itemLink(row.item)], ['Tipo de ficha', badge(row.catalog ? 'Catálogo' : 'Propia', row.catalog ? 'info' : 'mute')], ['Estado al corte', badge(row.state, fold(row.state) === 'activa' ? 'good' : 'mute')], ['Filas/variantes revisadas', n(row.variant_rows)], ['Celdas de atributos revisadas', n(row.attribute_cells)], ['Celdas vacías entre variantes', n(row.missing_cells)]]) + `<h4 class="detail-section-title">${row.missing_count ? 'Estos son todos los atributos que faltan en el export' : 'No se detectaron atributos vacíos'}</h4>` + (row.missing_count ? `<ul class="field-list">${row.missing_attributes.map(attribute => `<li><strong>${esc(attribute.label)}</strong><br>${attribute.occurrences.map(occurrence => `${esc(occurrence.sku || 'Sin SKU')} · hoja ${esc(occurrence.source_sheet)}, fila ${n(occurrence.source_row)}${occurrence.kind === 'placeholder' ? ' · texto de ejemplo sin completar' : ''}`).join('<br>')}</li>`).join('')}</ul>` : '') + `<p class="detail-note"><strong>Qué hacer:</strong> ${esc(row.action)} Estos son vacíos del archivo, no una confirmación de campos obligatorios. Revisá si cada atributo aplica al producto; no completes información que no puedas confirmar. Los campos de catálogo pueden requerir intervención de su responsable.</p><p class="detail-note">Completitud = celdas de atributos completas ÷ celdas revisadas × 100. Un atributo faltante en varias variantes se cuenta una sola vez en “Atributos pendientes”; el detalle muestra todas sus apariciones. Ventas se cruza por ID de publicación en Ventas UY; los ingresos de paquetes conservan la asignación proporcional del cierre.</p>`, summary: rows => `${n(rows.length)} publicaciones · ${n(sum(rows, 'missing_count'))} atributos pendientes, contados por publicación · ${n(rows.filter(row => row.has_net_sales && row.missing_count > 0).length)} publicaciones con ventas y pendientes · ${n(sum(rows, 'units_net'))} unidades operativas. No se duplican ventas entre variantes.`});
  el('catalog-actions').innerHTML = action('Empezar por fichas que venden', `${n(catalogSummary.sold_incomplete_publications)} publicaciones con ventas tienen ${n(catalogSummary.sold_missing_attributes)} atributos pendientes.`, 'Filtrar “Con ventas” y “Con campos pendientes”; ordenar por ingresos y abrir el detalle.', 'urgent') + action('Agrupar pedidos de corrección', catalog.missing_attributes_sold.slice(0, 3).map(field => `${field.label}: ${n(field.publications)} publicaciones`).join(' · '), 'Juntar la evidencia y solicitar los cambios aplicables al responsable del catálogo.', 'info');
  ...
  const additionalNotes = [
    ...
    {level: 'ok', title: 'Catálogo: publicaciones y variantes separadas', body: `${n(catalogSummary.source_variant_rows)} filas corresponden a ${n(catalogSummary.unique_publications)} publicaciones. El cruce de ventas ahora usa la publicación vendida; ${n(catalogSummary.sold_incomplete_publications)} con ventas tienen ${n(catalogSummary.sold_missing_attributes)} atributos pendientes. Los conteos anteriores por SKU/variante no eran publicaciones únicas.`},
  ...
  el('team-summary').innerHTML = [
    ...
    `Completar los atributos aplicables de ${n(catalogSummary.sold_incomplete_publications)} publicaciones que ya venden.`,
