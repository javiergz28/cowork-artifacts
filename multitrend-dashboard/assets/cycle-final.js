...
const plainCopy=value=>String(value??'').replace(/\bCOGS\b/g,'costo de los productos').replace(/\bledger(?:\s+contable)?\b/gi,'registro contable').replace(/P&L(?: comercial)?/g,'resultado comercial').replace(/\bliquida\b/g,'informa');
...
function mountTable(cfg){
  ...
  const sortOptions=[...new Map([...cfg.columns,...(cfg.sortOptions||[])].map(column=>[column.key,column])).values()];
  const mobileHeaders=window.matchMedia('(max-width:640px)');
  const syncHeaderFocus=()=>table.querySelectorAll('[data-sort]').forEach(button=>button.tabIndex=mobileHeaders.matches?-1:0);
  mobileHeaders.addEventListener('change',syncHeaderFocus);
  ...
  tools.innerHTML=`<div class="sort-controls"><label for="${cfg.table}-sort">Ordenar por</label><select id="${cfg.table}-sort">${sortOptions.map(column=>`<option value="${esc(column.key)}">${esc(column.label)}</option>`).join('')}</select><button type="button" class="btn sort-direction" aria-label="Cambiar sentido del orden de ${esc(cfg.title)}"></button></div><p class="table-status" id="${cfg.table}-status" role="status" aria-live="polite" aria-atomic="true"></p>`;
  wrapper.before(tools);
  const footer=document.createElement('div');footer.className='table-footer';
  footer.innerHTML=`<div class="page-size"><label for="${cfg.table}-size">Filas por página</label><select id="${cfg.table}-size"><option value="10">10</option><option value="20">20</option><option value="50">50</option><option value="all">Todas</option></select></div><div class="pagination"><button type="button" class="btn prev" aria-label="Página anterior de ${esc(cfg.title)}">← Anterior</button><span class="page-label"></span><button type="button" class="btn next" aria-label="Página siguiente de ${esc(cfg.title)}">Siguiente →</button></div>`;
  ...
  sizeSelect.addEventListener('change',()=>{pageSize=sizeSelect.value==='all'?Math.max(1,cfg.rows.length):Number(sizeSelect.value);page=1;render();});
...
const sourceRows=DATA.sources.map(source=>/report-(campaigns|placement)_/.test(source.file)?{...source,status:'Diferencias',note:plainCopy(source.note)+' La lectura ampliada identifica diferencias de cobertura frente a anuncios; ver conciliación en Ads.'}:source);
mountTable({table:'source-table',title:'Archivos del cierre',search:'source-search',rows:sourceRows,sort:'file',direction:'asc',unit:'archivos',searchKeys:['file','destination','period','status','note','sha'],filters:[{id:'source-filter',label:'Destino',get:row=>row.destination}],columns:[
