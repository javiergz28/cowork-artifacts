...
const server = createServer(async (req, res) => {
  ...
  handled = true;
  const child = spawn(process.execPath, [join(repoRoot, 'tools', 'build-protected-home.mjs')], {cwd:repoRoot,env:{...process.env,STATICRYPT_PASSWORD:password},stdio:'ignore'});
  child.once('error', () => { res.writeHead(500, {'content-type':'text/html; charset=utf-8'}); res.end(page('<p class="note error">No se pudo iniciar el cifrado. No se publicó nada.</p>')); server.close(); });
  child.once('exit', code => { const ok = code === 0; res.writeHead(ok ? 200 : 422, {'content-type':'text/html; charset=utf-8','cache-control':'no-store'}); res.end(page(ok ? '<p class="note"><strong>Listo.</strong> La portada se cifró localmente. La publicación segura continúa ahora.</p>' : '<p class="note error">No se generó una salida válida. No se publicó nada.</p>')); server.close(); });
});
