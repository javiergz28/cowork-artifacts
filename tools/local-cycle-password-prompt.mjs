...
const port = Number(process.argv[3] || 4182);
...
if (!/^\d{4}-\d{2}-\d{2}$/.test(cycle || '')) throw new Error('Indicá el ciclo como AAAA-MM-DD.');
if (!Number.isInteger(port) || port < 1024 || port > 65535) throw new Error('Puerto local inválido.');

const page = (message = '') => `<!doctype html><html lang="es"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Multitrend · Publicar ciclo</title><style>body{margin:0;min-height:100vh;display:grid;place-items:center;background:#f5f7f4;color:#0c0c0c;font:16px/1.5 Inter,system-ui,sans-serif}.box{width:min(440px,calc(100% - 40px);padding:30px;background:#fff;border:1px solid #dce5dd;border-radius:18px;box-shadow:0 18px 45px #0c0c0c16}.brand{font-size:13px;font-weight:900;letter-spacing:.1em}.brand i{color:#147d35;font-style:normal}h1{margin:10px 0;font-size:28px;letter-spacing:-.05em}p{color:#536057;font-size:14px}label{display:grid;gap:7px;font-size:12px;font-weight:800}input{padding:12px;border:1px solid #9bac9d;border-radius:9px;font:inherit}button{width:100%;margin-top:17px;padding:12px;border:0;border-radius:9px;background:#2bea60;color:#0c0c0c;font:inherit;font-weight:900;cursor:pointer}.note{margin-top:15px;padding:10px;background:#eaf9ed;border-radius:9px;color:#155c2d;font-size:12px}.error{background:#fff0ee;color:#8d2720}</style><main class="box"><div class="brand">MULTI<i>TREND</i></div><h1>Publicar ciclo ${cycle}</h1><p>Ingresá la contraseña compartida del panel. Se usa una sola vez para cifrar este ciclo en esta computadora; no se guarda ni sale de localhost.</p>${message}<form method="post"><label>Contraseña actual<input type="password" name="password" autocomplete="current-password" autofocus required></label><button type="submit">Cifrar el ciclo</button></form></main></html>`;
...
const server = createServer(async (req, res) => {
  ...
  if (!verified.success) {
    ...
    res.end(page('<p class="note error">Esa contraseña no abre el panel actual. No se publicó nada.</p>'));
    return;
  }
  handled = true;
  const child = spawn(process.execPath, [join(repoRoot, 'tools', 'build-protected-cycle.mjs'), cycle], {
    cwd: repoRoot,
  ...
  child.once('error', () => {
    ...
    res.end(page('<p class="note error">No se pudo iniciar el cifrado. No se publicó nada.</p>'));
    server.close();
  });
  child.once('exit', code => {
    const ok = code === 0;
    res.writeHead(ok ? 200 : 422, {'content-type':'text/html; charset=utf-8','cache-control':'no-store'});
    res.end(page(ok ? '<p class="note"><strong>Listo.</strong> El ciclo se cifró localmente. La publicación segura continúa ahora.</p>' : '<p class="note error">La contraseña no pudo generar una salida válida. No se publicó nada.</p>'));
    server.close();
  });
});
